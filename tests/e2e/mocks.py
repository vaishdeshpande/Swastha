...
inbound_graph always runs: language_router → voice_intake → (scheduler | prescription) → post_call
...
So the number of LLM calls per ainvoke() = (voice_intake loops) + (1 scheduler/rx call if intent resolved)
...
def llm_sequence(*responses: dict | str) -> MagicMock:
    ...
    for resp in responses:
        m = MagicMock()
        m.choices[0].message.content = json.dumps(resp) if isinstance(resp, dict) else resp
